...
// Route 6: GET /business_score/:zipcode/:category
const business_score = async function(req, res) {
  const reviewWeight = req.query.review_weight ?? 0.5;
  ...
  connection.query(`
    ...
    FROM (
      ...
      FROM (
        ...
        JOIN Business_Category BC ON B.id = BC.business_id
        JOIN Category C ON BC.category_id = C.id
        WHERE C.name = '${req.params.category}' AND B.zipcode = ${req.params.zipcode}
        GROUP BY zipcode
      ) a) final
  ...
  connection.query(`
    ...
    FROM (
      SELECT zipcode,
        ...
        median_rent_value,
        (CASE
          WHEN ${homeValueHigh} THEN (SELECT MAX(quintile) FROM home_value_quintile WHERE  median_home_value >= home_value)
          ELSE (6 - (SELECT MAX(quintile) FROM home_value_quintile WHERE  median_home_value >= home_value))
        END) AS home_value_score,
        (CASE
          WHEN ${rentValueHigh} THEN (SELECT MAX(quintile) FROM rent_value_quintile WHERE  median_rent_value >= rent_value)
...
module.exports = {
  ...
  us_statistics,
  business_score,
  housing_economics_demographics_score,
  business_category
}
